...
export function loadCampaign(name) {
  ...
  return campaignConfig(name, raw);
}
...
export function patchCampaign(name, patch) {
  if (!/^[a-zA-Z0-9_-]+$/.test(name)) throw new Error('Invalid campaign name');
  ...
  for (;;) {
    try { fs.mkdirSync(lock); break; }
      ...
      if (Date.now() > deadline) throw new Error('Campaign is busy; retry saving');
  ...
  try {
    ...
    const changes = typeof patch === 'function' ? patch(structuredClone(raw)) : patch;
    const next = { ...raw, ...changes };
    const cfg = campaignConfig(name, next);
    fs.writeFileSync(tmp, JSON.stringify(next, null, 2), { mode: 0o600 });
    fs.renameSync(tmp, file);
    return cfg;
  } finally { fs.rmSync(tmp, { force: true }); fs.rmdirSync(lock); }
}
