// Generated by scripts/build-prototype.mjs from src/role.js
(()=>{
...
const DEFAULT_EXCLUDE = ['recruiter', 'talent acquisition', 'headhunter'];
...
function guessSkills(text, max = 4, { family = '', learned = [], dropped = [] } = {}) {
  const t = String(text).toLowerCase();
  const counts = [];
...
function buildBoolean({ titles = [], domain = [], skills = [], anyOf = [], exclude = DEFAULT_EXCLUDE } = {}) {
...
function buildSearchUrl(boolean, geoUrns = []) {
...
function lookupGeo(name, { loose = false } = {}) {
...
function slugFor(title, location) {
...
function draftRole(text, known = {}, vocab = {}) {
...
const FAMILY_WORD = { engineering: 'software engineering', security: 'security', compliance: 'compliance', sales: 'sales or business development', devrel: 'developer relations' };
// Security engineers are engineers too; the title decides first, the spec only when the title is silent.
function familyOf(title, text = '') {
...
const genericTitle = t => /^(?:(?:senior|lead|principal|staff|junior)\s+)?(?:(?:software|backend|back-end|platform|infrastructure|full[- ]stack|systems?|application)\s+)?(?:engineer|developer)$/i.test(clean(t));

// A generic engineering title gets its near neighbours, picked by what the spec is about.
function relatedTitles(title, text, base = []) {
  const out = [...base];
...
const titleSkills = title => genericTitle(title) ? [] : TECH_SKILLS.filter(w => countOf(String(title).toLowerCase(), w));
function buildSkillsBoolean({ key = '', required = [], family = '', domain = [], text = '', hints = null, title = '', exclude = DEFAULT_EXCLUDE } = {}) {
...
function industryOf(boolean, fallback = []) {
